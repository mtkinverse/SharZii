...
Step 4:
set the env and run the daphne server:
export DJANGO_SETTINGS_MODULE=realTimeMessaging.settings [For linx]
set DJANGO_SETTINGS_MODULE=realTimeMessaging.settings [For windows]

Step 5 (Run the server):
daphne -b 0.0.0.0 -p 8000 realTimeMessaging.asgi:application

Additional Commands:
Force and check the path:
hash -r
echo $PATH
